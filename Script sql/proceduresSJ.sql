...
BEGIN
    ...
    BEGIN TRY
        ...
        FROM [sql_corp].[Corporativo].Sales.Customers AS customer
            ...
            LEFT JOIN [sql_corp].[Corporativo].Sales.Customers AS customerBillTo
...
LEFT JOIN [sql_corp].[Corporativo].Sales.Customers customer ON venta.CustomerID = customer.CustomerID
...
BEGIN
    ...
    BEGIN TRY
        ...
        FROM Sales.Invoices venta
        LEFT JOIN [sql_corp].[Corporativo].Sales.Customers customer
...
LEFT JOIN Sales.InvoiceLines AS lineas
    ON venta.InvoiceID = lineas.InvoiceID
LEFT JOIN [sql_corp].[Corporativo].Sales.Customers AS cliente
...
INNER JOIN Sales.InvoiceLines AS lineas
    ON venta.InvoiceID = lineas.InvoiceID
INNER JOIN [sql_corp].[Corporativo].Sales.Customers AS cliente
    ON venta.CustomerID = cliente.CustomerID
GROUP BY 
    YEAR(venta.InvoiceDate),
    cliente.CustomerID,
...
BEGIN
    ...
    BEGIN TRY
        ...
        -- Actualiza los grupos del producto
        IF @GruposProductoIDs IS NOT NULL AND LTRIM(RTRIM(@GruposProductoIDs)) <> ''
        BEGIN
            DELETE FROM Warehouse.StockItemStockGroups 
            WHERE StockItemID = @ProductoID;

            INSERT INTO Warehouse.StockItemStockGroups (StockItemID, StockGroupID)
            SELECT @ProductoID, value
...
CREATE PROCEDURE editarInventarioProducto
    @ProductoID INT,
...
BEGIN
    ...
    BEGIN TRY
        ...
        SET 
            QuantityOnHand = ISNULL(@CantidadDisponible, QuantityOnHand),
            BinLocation = ISNULL(@BinLocation, BinLocation),
            LastStocktakeQuantity = ISNULL(@LastStocktakeQuantity, LastStocktakeQuantity),
            LastCostPrice = ISNULL(@LastCostPrice, LastCostPrice),
            ReorderLevel = ISNULL(@ReorderLevel, ReorderLevel),
            TargetStockLevel = ISNULL(@TargetStockLevel, TargetStockLevel),
